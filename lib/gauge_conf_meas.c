...
void U1_extract(Gauge_Conf *GC, 
   ...
   // create filename
   sprintf(filename, "conf_u1_subg%d.dat%ld", subg, GC->update_index);

   // open file for writing
   FILE *subg_file = fopen(filename, "wb");
   if (subg_file == NULL) {
   ...
   for(lex=0;lex<param->d_volume;lex++)
      ...
      for(dir=0;dir<STDIM;dir++)
         ...
         //    }
         if(subg == 0){
            GC->u1_subg[si][dir] = GC->diag_proj[si][dir][0];
         }
         
         if(subg == 1){
            GC->u1_subg[si][dir] = -1*GC->diag_proj[si][dir][2];
         }
         
         // if(GC->u1_subg[r][dir] < -1*PI || GC->u1_subg[r][dir] > PI){
         //    printf("%lf ",GC->u1_subg[r][dir]);
         // }
         // printf("%lf ",GC->u1_subg[r][dir]);

         // print in little endian format
         fwrite(&GC->u1_subg[si][dir], sizeof(double), 1, subg_file);
