...
///////////// New Section/////////
void perform_measure_u1subg(Gauge_Conf const * const GC,
  ...
  // monopole observables
   if(param->d_mon_meas == 1)
     ...
     double xi2_1[STDIM] = {1.0, 1.0, 1.0, 1.0};

     init_gauge_conf_from_gauge_conf(&helperconf, GC, param);
     alloc_diag_proj_stuff(&helperconf, param);

     printf("MAG fixing with xi = 1.0 \n");
     // MAG gauge fixing
     max_abelian_gauge_fix(&helperconf, geo, param, xi2_1);
     printf("MAG fixing DONE\n");

     printf("MAG fixing with xi = 3.453\n");
     // MAG gauge fixing
     max_abelian_gauge_fix(&helperconf, geo, param, xi2);
     printf("MAG fixing DONE\n");
     ...
     for(subg=0; subg<subgnum; subg++)
        {
        // extract the abelian component of the link and save it to GC->u1_subg
...
void real_main(char *in_file)
    ...
    time_t time1, time2;
    double xi = 3.453 * 3.453;
    // double xi = 1.0;
    double xi2[STDIM] = {xi, 1.0, 1.0, 1.0};  
      ...
      //  update(&GC, &geo, &param);
       ...
       if(count % param.d_measevery ==0 && count >= param.d_thermal)
         ...
         perform_measures_localobs(&GC, &geo, &param, datafilep, monofilep);
          perform_measure_u1subg(&GC, &geo, &param, xi2);
         }
